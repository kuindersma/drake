...
%   [f1,df1] = tmp1(h(i),x(:,i),u(:,i),l(:,i));
%   [f2,df2] = geval(tmp1,h(i),x(:,i),u(:,i),l(:,i),struct('grad_method','numerical'));
%   valuecheck(df1,df2,1e-3);
  ...
  tmp1 = @(l,a) traj_opt.lagrange_eq(l,a); 
  [f1,df1] = tmp1(l,a);
  [f2,df2] = geval(tmp1,l,a,struct('grad_method','numerical'));
  valuecheck(df1,df2,1e-3);
