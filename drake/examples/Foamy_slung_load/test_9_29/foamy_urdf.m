classdef foamy_urdf < DrakeSystem
  ...
  methods
    ...
    function obj = foamy_urdf(options)
      ...
      obj.plane_link_id = findLinkId(obj.plane_rbm,'airplane_body');
     % obj = compile(obj);
    ...
    function [f,df] = dynamics(obj,t,x,u)
        ...
        %q = x(1:9);
        %v = x(10:end);
        [H,C_times_v,G,B] = obj.plane_rbm.manipulatorEquations(x(1:7),x(8:end));
        [xdot,F,T,ret_T] = foamy_dynamics(t,x,u);
        %kinsol = doKinematics(obj.plane_rbm,x(1:obj.plane_rbm.getNumPositions));
        kinsol = doKinematics(obj.plane_rbm,x(1:obj.plane_rbm.getNumPositions) , x(obj.plane_rbm.getNumPositions+1:end), struct('compute_gradients', true));
        %kinsol = doKinematics(obj.plane_rbm,x(1:obj.plane_rbm.getNumPositions) , x(obj.plane_rbm.getNumPositions+1:end), struct('compute_gradients', true));
        ...
        %J = obj.plane_rbm.geometricJacobian(kinsol,2,1,2,0);
        [J,indices,~] = obj.plane_rbm.geometricJacobian(kinsol,1,2,1);
        quat = q;
        %G_trans = obj.omega_to_qdot(q);
        %J_quat = 2*J(4:7,:)'*G_trans;
        %J = [J(1:3,:);J_quat'];
       ...
       % F_gen = [F_quat;F];
        temp = J'*F_gen;
        temp2 = [temp(4:6);temp(1:3)];
        qdd = H\(-C_times_v-G+temp2); %[wx,wy,wz,vx,vy,vz,theta_dot,phi_dot]'
        %f = [v;.5*[-q(2:4)'*w; q(1)*w + cross(q(2:4),w)];x(16:17);qdd];
        f = [v;.5*[-q(2:4)'*w; q(1)*w + cross(q(2:4),w)];qdd(4:6);qdd(1:3)];
        if max(abs(f)) > 1e5
           indicator = 1;
        else
        end
